...
class TestProduct(TestCase):
    ...
    def test_good_search_view(self):
        response = self.client.get(reverse('blog:search'), {'query': 'name'})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'blog/search.html')

    def test_product_view(self):
        prod_id = Product.objects.first().id
        response = self.client.get(reverse('blog:product',
                                           args=[prod_id]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'blog/product.html')

    def test_substitute_view(self):
        sub_id = Product.objects.last().id
        response = self.client.get(reverse('blog:substitute',
                                           args=[sub_id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_save_logged_in_view(self):
        ...
        sub_id = Product.objects.last().id
        response = self.client.get(reverse('blog:save',
                                           args=[prod, sub_id]))
        self.assertEqual(response.status_code, 302)
        self.assertTemplateUsed(redirect('index.html'))
    ...
    def test_favorite_logged_in_view(self):
        user = User.objects.first()
        self.client.force_login(user)
        response = self.client.get(reverse('blog:favorite'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_logged_in_view(self):
        ...
        response = self.client.get(reverse('blog:delete',
                                           args=[fav]))
        self.assertEqual(response.status_code, 302)
        self.assertTemplateUsed(redirect('index.html'))
